(function()
{
  window.onload = function()
  {
    ...
    formulario.nombreU.onblur = function() {
      revisarUsuario(this);
    }
    formulario.correo.onblur = function() {
      revisarCorreo(this);
    }

    function revisarUsuario(usuario) { //Buscar en la DB la cédula ingresada
      var post = "usuario=" + usuario.value;
      var xml;
      ...
      xml.onreadystatechange = function() {
        if (xml.readyState === 4 && xml.status === 200) {
          if (usuario.nextElementSibling != null) {
          ...
          { //Si no se cumple la expresion muestra un mensaje de error
            ...
            if (xml.responseText == "1") {
              mensaje = "Este Usuario se encuentra registrado";
              usuarioValido = false;
            }
            if (usuario.nextElementSibling == null) {
              p.innerHTML = mensaje;
              usuario.parentNode.appendChild(p);
            }
          }
          else {
            usuarioValido = true;
      ...
      xml.open('POST', '/ublc/controlador/ajax/buscarUsuario.php', true);
      xml.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
      xml.send(post);
    }
  }
  function revisarCorreo(correo) { //Buscar en la DB la cédula ingresada
    var post = "correo=" + correo.value;
    var xml;
    if (window.XMLHttpRequest) {
      xml = new XMLHttpRequest();
    }
    ...
    xml.onreadystatechange = function() {
      if (xml.readyState === 4 && xml.status === 200) {
        if (correo.nextElementSibling != null) {
          correo.nextElementSibling.remove();
        }
        ...
        { //Si no se cumple la expresion muestra un mensaje de error
          ...
          var mensaje;
          if (xml.responseText == "1") {
            mensaje = "Este Correo se encuentra registrado";
            correoValido = false;
          }
          if (correo.nextElementSibling == null) {
            p.innerHTML = mensaje;
            correo.parentNode.appendChild(p);
          }
        }
        else {
          correoValido = true;
    ...
    xml.open('POST', '/ublc/controlador/ajax/buscarUsuarioCorreo.php', true);
    xml.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xml.send(post);
  }
  ...
  enviando = function()
  {
    ...
    formulario.correo.blur();
    formulario.nombreU.focus();
    formulario.nombreU.blur();
    if (nombreVerificar() && cedulaVerificar() && correoVerificar() && correoValido && validarUsuario() && usuarioValido && validarClave())
    {
      formulario.submit();
    }
