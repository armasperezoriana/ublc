 (function()
 {
  window.onload = function() {
    //Declaraciones de variables y eventos
    var enviar, limpiar;
    var formulario;
    var formularioDos;
    formularioDos = document.getElementById("formulario_dos");
    var cedulaPersona = false, cedulaTitular = false;
    revisarCedula(formularioDos.cedula);
    declararFormulario(formularioDos);
  ...
  function declararFormulario(form) {
    ...
    //Validar cédulas
    form.cedulaTitular.onkeyup = function() {
      var expresion = /^[0-9]{7,9}$/;
      var mensaje = "Ingrese una Cédula de Titular válida";
      validarCampo(expresion, this, mensaje);
    };
    form.cedula.onkeyup = function() {
      var expresion = /^[0-9]{7,9}\-{0,1}[0-9]{0,1}$/;
    ...
    form.cedulaTitular.onblur = function() {
      revisarCedulaTitular(this);
    };
    //Botones
    form.enviar.onclick = enviandoBene;
    if (form.limpiar != null) {
      form.limpiar.onclick = limpiandoBene;
    }
  ...
  limpiandoBene = function()
  {
    formulario = document.getElementById("formulario_dos");
    formulario.reset();
  }
  enviandoBene = function()
  {
    formulario = document.getElementById("formulario_dos");
    if (cedulaTitVerificar() && parentescoVerificar() && cedulaBeneVerificar() && nombreVerificar() && cedulaPersona && cedulaTitular
      && nacimientoVerificar() && sexoVerificarBene() && direccionVerificar() && telefonosVerificar()
      && correoVerificar())
    {
      formulario.submit();
    }
  ...
  parentescoVerificar = function() {
    if (formulario.parentesco.value == "") {
      alert("Seleccione un Parentesco");
      formulario.parentesco.focus();
      return false;
    }
    else {
      return true;
